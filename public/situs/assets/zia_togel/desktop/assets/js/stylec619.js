$(function () {
  ...
  owlbank.owlCarousel({ autoPlay: 5000, items: 5, itemsDesktop: false });
  ...
  $('a[data-nivobox^="true"]').nivoLightbox({ effect: "fade" });
  $(".twitter-feed").tweet({
    username: "envato",
  ...
  clientsCarousel.owlCarousel({ items: 5, pagination: false });
  ...
  clientsFeedbackCarousel.owlCarousel({ singleItem: true, pagination: false });
  ...
  $(".flickr-feed ul").jflickrfeed(
    {
      limit: 12,
      qstrings: { id: "12625534@N00" },
      itemTemplate:
        '<li><a href="{{image_b}}" title="{{title}}" data-nivobox="true" data-lightbox-gallery="photo-stream"><img src="{{image_s}}" alt="{{title}}" /></a><span class="photo-stream-overlay"><i class="fa fa-arrows-alt"></i></span></li>',
    },
    function (data) {
      $(".flickr-feed li a").nivoLightbox({ effect: "fade" });
    }
  );
  ...
  $(".progress").each(function () {
    $(this).appear(function () {
      $(this)
        .find(".progress-bar")
        .css({ width: $(this).find(".progress-bar").data("percent") });
    });
  });
...
$(document).ready(function () {
  $("#liscroll").liScroll();
  $("#headerFixed").fixedHeaderTable({ footer: true, altClass: "odd" });
  $("#modal-trigger").click();
});
